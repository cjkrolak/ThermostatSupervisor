...
def download_ssl_certificate(hostname: str, port: int = 443) -> pathlib.Path:
    ...
    util.log_msg(
        f"Downloading SSL certificate from {hostname}:{port}",
    ...
    try:
        ...
        util.log_msg(
            f"SSL certificate downloaded to {cert_path}",
            mode=util.STDOUT_LOG
        )
        return cert_path
...
def import_ssl_certificate_to_system(cert_path: pathlib.Path) -> bool:
    ...
    try:
        ...
        util.log_msg(
            f"Failed to import SSL certificate: {e}",
            mode=util.STDERR_LOG
        )
        return False
...
def _import_cert_linux(cert_path: pathlib.Path) -> bool:
    ...
    for cert_dir in cert_dirs:
        ...
        if cert_dir_path.exists() and cert_dir_path.is_dir():
            try:
                ...
                util.log_msg(
                    f"Certificate imported to {target_path}",
                    mode=util.STDOUT_LOG
                )
                imported = True
                break
    ...
    if not imported:
        ...
        util.log_msg(
            "Could not import to system store, using environment variable "
            "fallback",
            mode=util.STDOUT_LOG
        )
...
def _import_cert_windows(cert_path: pathlib.Path) -> bool:
    ...
    try:
        ...
        util.log_msg(
            "Certificate imported to Windows certificate store",
            mode=util.STDOUT_LOG
        )
        return True
...
def download_and_import_ssl_certificates(servers: List[Tuple[str, int]]) -> bool:
    ...
    for hostname, port in servers:
        try:
            ...
            util.log_msg(
                f"Error processing {hostname}:{port}: {e}",
                mode=util.STDERR_LOG
            )
            success = False

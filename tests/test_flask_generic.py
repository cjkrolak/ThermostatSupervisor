...
Classes:
    TestFlaskGeneric: Contains unit tests for the `schedule_ipban_black_list_report`
                      and `print_ipban_black_list_with_timestamp` functions.
Methods:
    test_schedule_ipban_black_list_report(MockAPScheduler):
        Tests the `schedule_ipban_black_list_report` function to ensure it schedules
        the IP ban black list report correctly with different debug modes.
    test_print_ipban_black_list_with_timestamp(mock_datetime):
        Tests the `print_ipban_black_list_with_timestamp` function to ensure it prints
        the IP ban black list with the correct timestamp.
"""
...
from thermostatsupervisor.flask_generic import (
    print_ipban_black_list_with_timestamp,
    schedule_ipban_black_list_report,
)
...
class TestFlaskGeneric(utc.UnitTest):
    ...
    1. `test_print_ipban_black_list_with_timestamp`:
        - Tests the `print_ipban_black_list_with_timestamp` function to ensure it prints
        - Mocks the current datetime to return a specific timestamp and verifies that
          ...
          timestamp and the IP ban black list.
    2. `test_schedule_ipban_black_list_report_debug_mode`:
        - Tests the `schedule_ipban_black_list_report` function to ensure that it
          schedules.
        - Sets `debug_mode` to True and verifies that the print function is called with
          ...
          every 1.0 minutes.
    3. `test_schedule_ipban_black_list_report_normal_mode`:
        - Tests the `schedule_ipban_black_list_report` function in normal mode.
    ...
    def setUp(self):
        ...
        IP ban object and sets up its return value for the get_black_list method.
        Attributes:
        ...
        self.mock_ip_ban.get_black_list.return_value = {
            "192.168.1.1": {
                "count": 3,
    ...
    def test_print_ipban_black_list_with_timestamp(self, mock_print):
        """
        Test the print_ipban_black_list_with_timestamp function to ensure it prints
        the IP ban black list with the correct timestamp.
        Args:
        ...
        Act:
            - Call the print_ipban_black_list_with_timestamp function with the mocked
              IP ban list.
        Assert:
            - Verify that the print function is called once with the expected output
              string containing the timestamp and the IP ban black list.
        """
        ...
        with patch("datetime.datetime") as mock_datetime:
            ...
            # Act
            print_ipban_black_list_with_timestamp(self.mock_ip_ban)

            # Assert
            mock_print.assert_called_once_with(
                f"{expected_timestamp}: ip_ban black list: "
                f"{self.mock_ip_ban.get_black_list()}"
            )

    @patch("builtins.print")
    def test_schedule_ipban_black_list_report_debug_mode(self, mock_print):
        """
        Test the `schedule_ipban_black_list_report` function to ensure that it schedules
        the IP ban black list report correctly when debug mode is enabled.
        Args:
            mock_print (Mock): Mock object for the print function.
        Arrange:
        ...
        Act:
            ...
            - Call `schedule_ipban_black_list_report` with the mock IP ban object and
              debug mode.
        Assert:
        ...
        with patch("apscheduler.schedulers.background.BackgroundScheduler"):
            schedule_ipban_black_list_report(self.mock_ip_ban, debug_mode)
    ...
    @patch("builtins.print")
    def test_schedule_ipban_black_list_report_normal_mode(self, mock_print):
        """
        Test the `schedule_ipban_black_list_report` function in normal mode.
        This test verifies that the `schedule_ipban_black_list_report` function
        schedules the IP ban black list report correctly when the debug mode is

...
class FunctionalIntegrationTest(IntegrationTest, utc.FunctionalIntegrationTest):
    ...
    def test_deviation_functionality(self):
        ...
        try:
            ...
            self.assertFalse(Zone.has_deviation_data(), "Should have no deviation data initially")
            ...
            self.assertTrue(Zone.has_deviation_data(), "Should have deviation file after creation")
            ...
            self.assertTrue(Zone.has_deviation_data("display_temp"), "Should have temp deviation")
            self.assertTrue(Zone.has_deviation_data("display_humidity"), "Should have humidity deviation")
            self.assertTrue(Zone.has_deviation_data("heat_setpoint"), "Should have heat setpoint deviation")
            self.assertTrue(Zone.has_deviation_data("cool_setpoint"), "Should have cool setpoint deviation")
            
            self.assertEqual(Zone.get_display_temp(), test_temp, "Should return deviated temperature")
            self.assertEqual(Zone.get_display_humidity(), test_humidity, "Should return deviated humidity")
            self.assertEqual(Zone.get_heat_setpoint_raw(), test_heat_sp, "Should return deviated heat setpoint")
            self.assertEqual(Zone.get_cool_setpoint_raw(), test_cool_sp, "Should return deviated cool setpoint")
            
            # Test 5: Test individual value retrieval
            self.assertEqual(Zone.get_deviation_value("display_temp"), test_temp, "Should retrieve specific deviation value")
            self.assertIsNone(Zone.get_deviation_value("nonexistent_key"), "Should return None for missing key")
            self.assertEqual(Zone.get_deviation_value("nonexistent_key", "default"), "default", "Should return default for missing key")
            
            # Test 6: Clear deviation data
            Zone.clear_deviation_data()
            self.assertFalse(Zone.has_deviation_data(), "Should have no deviation data after clearing")
            ...
            humidity_diff = abs(current_humidity - Zone.get_parameter("display_humidity"))
            
            self.assertLessEqual(temp_diff, emulator_config.NORMAL_TEMP_VARIATION,
                               "Temperature should be within normal variation after clearing deviation")
            self.assertLessEqual(humidity_diff, emulator_config.NORMAL_HUMIDITY_VARIATION,

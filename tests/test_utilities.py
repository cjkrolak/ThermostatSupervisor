    ...
    def test_is_host_on_local_net_additional_coverage(self):
        ...
        with patch('socket.gethostbyname') as mock_gethostbyname:
            with patch('builtins.print') as mock_print:
                ...
                mock_print.assert_called_with("host test_host found at 192.168.1.1 on local net")
        ...
        with patch('socket.gethostbyaddr') as mock_gethostbyaddr:
            with patch('builtins.print') as mock_print:
                mock_gethostbyaddr.return_value = ("different_host", [], ["192.168.1.1"])
                
                result, ip = util.is_host_on_local_net("expected_host", "192.168.1.1")
                
                self.assertFalse(result)
                self.assertIsNone(ip)
                mock_print.assert_called_with("DEBUG: expected host=expected_host, actual host=('different_host', [], ['192.168.1.1'])")
